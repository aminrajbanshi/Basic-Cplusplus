...
void add_to_start(linked_list &list, int value)
{
    ...
    added_node = new_node(value, list, nullptr, list.first);
...
/**
 * Insert new node after the selected node.
 * 
 * @param selected_node     The node to insert the new node after
...
void insert_after(node selected_node, int value)
{
    linked_list *list;
    node after;
    node new_node_to_add;

    list = selected_node->in_list;
    after = selected_node->next;

    if (list->last == selected_node)
    {
        add_to_end(*list, value);
    }
    else
    {
        new_node_to_add = new_node(value, *list, selected_node, after);

        after->prev = new_node_to_add;
        selected_node->next = new_node_to_add;
    }
}
...
/**
 * Print all of the node in this linked list in reverse order.
 * 
 * @param list      The list to print the nodes from.
 */
...
/**
 * The reverse for each function loops over the elements of a list
 * in reverse order and executes a visitor function on each element.
 * 
 * @param list  The list of nodes to visit
 * @param fn    The function to call
 */
...
/**
 * Print all of the nodes from the list in reverse order (using reverse for each function).
 * 
 * @param list      The list of nodes to print
 */
void reverse_print_with_visitor(const linked_list &list)
{
...
/**
 * Calculates length of the list (number of nodes in the entire list).
 * 
...
int main()
{
    ...
    add_to_start(my_list, 40);
    add_to_start(my_list, 50);
    add_to_start(my_list, 60);
    print_all(my_list);

    write_line("---TEST: Reverse print all---");
    reverse_print_all(my_list);

    write_line("---TEST: Reverse print all with visitor---");
    reverse_print_with_visitor(my_list);
    ...
    //Test insert after...
    write_line("---TEST: Insert after---");
    found = find_first(my_list, 50);
    insert_after(found, 90);
    print_all(my_list);

    //Test find and delete...
    write_line("---TEST: Find and delete---");
    found = find_first(my_list, 20);
    delete_node(found);
    print_all(my_list);

    // Delete start and end as well...
    write_line("---TEST: Delete start---");
    found = find_first(my_list, 60);
    delete_node(found);
    print_with_visitor(my_list);

    write_line("---TEST: Delete end---");
    found = find_first(my_list, 30);
    delete_node(found);
    print_all(my_list);

    //Add a new
    write_line("---TEST: Add new to end---");
    add_to_end(my_list, 100);
    print_all(my_list);
